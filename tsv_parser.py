...
def _file_name(row):
    return "%s/%s_%s" % (row['folder'], row.name, (zlib.crc32(row['image_url'].encode('utf-8')) & 0xffffffff))
...
def check_download(row):
    ...
    try:
        ...
        response = requests.head(row['image_url'], stream=False, timeout=5, allow_redirects=True, headers=headers)
...
def download_image(row):
    ...
    try:
        ...
        response = requests.get(row['image_url'], stream=False, timeout=10, allow_redirects=True, headers=headers)
        row['status'] = response.status_code
        #row['headers'] = dict(response.headers)
...
def open_tsv(fname, folder):
    ...
    df = pd.read_csv(fname, sep='\t', names=["language", "image_url", "ref_dsc"], usecols=[0, 2, 6]) #lang, imgurl, ref desc
    df = df.groupby('language').get_group('es') 
    df['folder'] = folder
...
'''
data_name = "training"
df = open_tsv("wit_dataset/wit_v1.train.all-00000-of-00010.tsv", data_name)
...
df.to_csv("downloaded_%s_report.csv.gz" % data_name, compression='gzip', sep=',', index=False)
print("Saved.")
